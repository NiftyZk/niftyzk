...
const { CIRCUITSDIR, LIBPATH, TESTPATH, COMPILEDDIR, DEVJSPATH } = require("../paths.js");
...
const { getHotReloadDevFile } = require("../templates/js/hotreload.js");
    ...
    inquirer.prompt([
        {
            type: "list",
            name: "merkletree",
            message: "Do you want a merkle tree? Select fixed for a fixed size merkle tree or none",
...
async function genJs(extraPublicInputsArr, hashfunc, merkletree) {
    ...
    const hotreload = getHotReloadDevFile(hashfunc, merkletree, extraPublicInputsArr);
    fs.writeFileSync(path.join(LIBPATH, "index.js"), lib)
    fs.writeFileSync(path.join(DEVJSPATH), hotreload);
