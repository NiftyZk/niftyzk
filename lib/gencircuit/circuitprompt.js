...
const { getJsLib, getTests, getPlonkTests } = require("../templates/js/templates.js");
...
function circuitPrompts(dirname, isPlonk) {
    inquirer.prompt([
        {
            type: "list",
            name: "scheme",
        ...
        if (addpubinputs) {
            ...
            await genJs({ extraPublicInputsArr: publicInputsArr, hashfunc, merkletree, scheme, dirname, isPlonk })

        } else {
            genCircuits({ publicInputsArr: [], hashfunc, merkletree, scheme, dirname })
            await genJs({ extraPublicInputsArr: [], hashfunc, merkletree, scheme, dirname, isPlonk })
        }
...
async function genJs({ extraPublicInputsArr, hashfunc, merkletree, scheme, dirname, isPlonk }) {
    ...
    const lib = getJsLib(scheme, extraPublicInputsArr, hashfunc, merkletree, isPlonk)
    const inputfile = getInputsFile(scheme, hashfunc, merkletree, extraPublicInputsArr)

    if (isPlonk) {
        const mainDirTwoUp = path.resolve(__dirname, '../..');
        const pkgDir = path.resolve(mainDirTwoUp, "rust", "pkg");
        const destPkg = path.resolve(libpath, "plonkit");
        fs.cp(pkgDir, destPkg, { recursive: true }, (err) => {
            if (err) {
