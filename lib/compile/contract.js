...
async function generateTestProof(circuitFileName, isPlonk) {
    ...
    if (isPlonk) {
        p = await plonk.fullProve(input, wasmFilePath, zkeyPath);

    } else {
        p = await groth16.fullProve(input, wasmFilePath, zkeyPath);
    ...
    let result
    if (isPlonk){
        result = await plonk.verify(verificationKey, publicSignals, proof);
    }else {

        result = await groth16.verify(verificationKey, publicSignals, proof);
    }
