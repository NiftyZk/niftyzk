...
async function generateTestProofForPlonk(wasmFilePath, setupFile, r1csFile) {
    let myModule;
    const modulePath = path.join(process.cwd(), "test", "input.js")
    await import(modulePath).then(module => myModule = module);
    ...
    let p = await wasm.prove(setupFile, r1csFile, witnessPath, JSON.stringify(input))
...
async function verifyTestProofForPlonk(vkey_bin, proof_bin) {
    ...
    let valid = await wasm.verify(vkey_bin, proof_bin)
...
async function genContract(options, isPlonk) {
    ...
    if (!isPlonk) {
        ...
        //TODO: IMPORTANT! Need to prompt for setupFile to select the correct one
        const setupFile = path.join(UNIVERSALSETUPFILESDIR, "setup_2^20.key")

        const testProof_bin = await generateTestProofForPlonk(witnessWasmPath, setupFile, r1csPath)
