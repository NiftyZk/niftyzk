...
//TODO: Contract path comes from cli input!
...
async function genContract(options) {
    ...
    if (options.ark) {
        generateArkGroth16(verifcationKeyPath, testProofPath, CONTRACTPATH);
    } else if (options.bellman) {
        generateBellmanGroth16(verifcationKeyPath, testProofPath, CONTRACTPATH);
    }
}
...
async function generateArkGroth16(verificationKeyPath, testProofPath, CONTRACTPATH) {
...
async function generateBellmanGroth16(verificationKeyPath, testProofPath, CONTRACTPATH) {
    const verificationKeyBuff = fs.readFileSync(verificationKeyPath).toString();
    const testProofBuff = fs.readFileSync(testProofPath).toString();
