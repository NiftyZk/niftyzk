...
function compileCircuits(filePath) {
    ...
    inquirer.prompt([
        ...
        await zKey.newZKey(r1csPath, ptauPath, zkeyPath, { error: console.log, info: console.log, debug: console.log })

        writeUsedPtauFileName(filename)
...
function writeUsedPtauFileName(filename) {
    fs.writeFileSync(path.join(ZKEYSDIR, "ptau.txt"), filename)
}
