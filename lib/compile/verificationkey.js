...
async function verificationKey(final) {
    ...
    await zkeyExportVKey(zkeyPath, verificationKeyPath, zkeyname)
}

async function zkeyExportVKey(zKeyFilePath, vKeyFileName, zkeyName) {

    Logger.setLogLevel("DEBUG");

    const vKey = await zKey.exportVerificationKey(zKeyFilePath, logger);

    await bfj.write(vKeyFileName, stringifyBigInts(vKey), { space: 1 }).then(() => {
        verificationkey_meta(`circuits/compiled/zkeys/${zkeyName}`)
    });
...
function findFirstZkey() {
    ...
    if (firstKey.length === 0) {
        throw new Error("Can't find file ending with 0000.zkey")
    }
    return firstKey[0]
}
