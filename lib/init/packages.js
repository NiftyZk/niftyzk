...
const niftyCircuitJsonName = "niftycircuit.json";
const niftyCircuitJsonContents = JSON.stringify({ privateInputs: [], publicInputs: [] })
...
async function setupWithCurrentDir() {
    ...
    writeCircuitMetadataFile(path.join(process.cwd(), niftyCircuitJsonName))
}
...
async function setupWithNewDir(dirname) {
    ...
    writeCircuitMetadataFile(path.join(dirname, niftyCircuitJsonName))
...
function writeCircuitMetadataFile(filePath) {
    if (!Fs.existsSync(filePath)) {
        Fs.writeFileSync(filePath, niftyCircuitJsonContents)
    }
}
