...
const gitignored = ["ptau", "node_modules", "private"]


async function setupWithCurrentDir(plonk) {
...
//This runs in the current working directory
async function checkPKGJsonForPlonk() {

    if (packageJsonExists()) {
        const pkgjson = await PackageJson.load(process.cwd())
        try {
            return pkgjson.content.myconfig.usePlonk === true
        } catch (err) {
            return false
        }
    } else {
...
async function setupWithNewDir(dirname, plonk) {
    ...
    genReadme(path.join(dirname, "readme.md"), plonk ? true : false)
}
...
function genReadme(readmepath, isPlonk) {
    if (!Fs.existsSync(readmepath)) {
        Fs.writeFileSync(readmepath, readmefile.content(isPlonk))
    }
}
...
module.exports = { setupWithCurrentDir, setupWithNewDir, checkIfCircomIsInstalled, checkPKGJsonForPlonk }