...
/**
 * The mutated tree returns this result which contains the serializableLeaves and the result of the update
 * @typedef {Object} MutateTreeResult
 * @property {Array<leaves>} serializableLeaves -- The serializable leaves 
 * @property {object} updateResult -- The result of the update
 */
 ...
 * @returns {MutateTreeResult} - Returns the mutate serializable leaves
 */

export async function mutateTree(action, key, value, serializableLeaves, smt) {
    if (action === "insert") {

        const updateResult = await smt.insert(smt.F.e(key), smt.F.e(value))

        serializableLeaves.push({ key, value })

        return { serializableLeaves, updateResult }

    } else if (action === "delete") {

        const updateResult = await smt.delete(smt.F.e(key));
        ...
        const updateResult = await smt.update(smt.F.e(key), smt.F.e(value));
...
it("SMT serialized leaves, insert, update, delete", async function () {
    ...
    let mutateResult = await mutateTree("insert", newEntry.key, newEntry.value, leaves, tree);

    leaves = mutateResult.serializableLeaves;
    ...
    mutateResult = await mutateTree("update", updateEntry.key, updateEntry.value, leaves, tree);
    ...
    mutateResult = await mutateTree("delete", deleteEntry.key, deleteEntry.value, leaves, tree)
