...
import { initializeTree, mutateTree } from "../lib/smt.js";
...
${initializeExtraInputs.join("\n")}
    ...
    const accountAddress = await getAddressFromPubkey(eddsa,account.pubKey);
    ...
    const INCLUSION = true; // Set to false for exclusion test
     ...
     if (INCLUSION) {
        ...
        const exampleAddress = await getAddressFromPubkey(eddsa,exampleAccount.pubKey);
...
${initializeExtraInputs.join("\n")}
    ...
    const accountAddress = await getAddressFromPubkey(account.pubKey${addTestsK(hashfunc, "arg")});
    ...
    const INCLUSION = true; // Set to false for exclusion test
     ...
     if (INCLUSION) {
        ...
        return {
            ...
            R8x: signatureParameters.R8x,
            R8y: signatureParameters.R8y,
            root: tree.F.toObject(tree.root),${addTestsK(hashfunc, "proofArg")}
            siblings,
            fnc: 0,
            oldValue: 0,
            oldKey: 0,
        ...
        const exampleAccount = generateAccount(eddsa);
        const exampleAddress = await getAddressFromPubkey(exampleAccount.pubKey${addTestsK(hashfunc, "arg")});
        const exampleTreeContent = [{ key: tree.F.e(exampleAddress), value: 0n }];
        //The tree is initialized with empty content, the signer's key is excluded
        await initializeTree(exampleTreeContent, tree);
        const foundKey = await tree.find(tree.F.e(1n)) // A key that doesn't exist in the exampleTreeContent
    ...
    switch (ACTION) {
        case "insert": {
            ...
            return {
                ...
                S: signatureParameters.S,
                R8x: signatureParameters.R8x,
                R8y: signatureParameters.R8y,${addTestsK(hashfunc, "proofArg")} 
                fnc: insertFNC,
                oldRoot: tree.F.toObject(updateResult.oldRoot),
                siblings,
                oldKey: updateResult.isOld0 ? 0 : tree.F.toObject(updateResult.oldKey),
