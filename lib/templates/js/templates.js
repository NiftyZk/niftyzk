...
function getHashers(hashfunc) {
    switch (hashfunc) {
        case "mimc7":
            return getMimc7Hashers();
        case "mimcsponge":
            return getMimcSponge();
        case "poseidon":
            return getPoseidonHashers();
        case "pedersen":
            return getPedersenHashers();
        default:
            return ``
    }
}

function getPedersenHashers(){
    return `/**
...
function getGenerateProof(extraPublicInputs, hashfunc) {
 ...
 */ 
export async function computeProof({secret, nullifier,${addKVar(hashfunc)} publicInputs, snarkArtifacts}){
    const input = {
      //Private inputs
      secret,
      nullifier,
...
function getJsLib(extraPublicInputs, hashfunc) {
    const imports = getImports(hashfunc);
    const random = getRandom();
    const hashers = getHashers(hashfunc);
    const generateProof = getGenerateProof(extraPublicInputs, hashfunc);
