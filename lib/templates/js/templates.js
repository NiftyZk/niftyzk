...
function getHashFuncImport(hashfunc, scheme) {
    const importEddsa = scheme === "eddsa" || scheme === "eddsa-merkle" ? `buildEddsa,` : ""
    switch (hashfunc) {
        case "mimc7":
...
function getTests(extraPublicInputs, hashfunc) {
    ...
    it("test commitment-reveal scheme", async function(){
        const {secret,nullifier,${addTestsK(hashfunc, "justk")}commitmentHash,nullifierHash,${extraPublicInputs.join(",")}} = await getInput();
...
function getJsLib(scheme, extraPublicInputs, hashfunc, merkletree) {
    if (scheme === "crs" || scheme === "crs-merkle") {
        ...
        const generateProof = eddsa_generateProof(extraPublicInputs, hashfunc, merkletree);
        const verifyProof = getVerifyProof();
