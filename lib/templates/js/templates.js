...
function getImports(hashfunc, scheme, isPlonk) {
    return `
import { utils } from "ffjavascript";
import crypto from "crypto";
import assert from "assert";
${getHashFuncImport(hashfunc, scheme)}
import { ${isPlonk ? "plonk" : "groth16"} } from "snarkjs";
...
function getGenerateProof(extraPublicInputs, hashfunc, merkletree, isPlonk) {
...
function getVerifyProof(isPlonk) {
    return `/**
...
export function verifyProof({verificationKey, proof, publicSignals }) {
    return ${isPlonk ? "plonk" : "groth16"}.verify(
        verificationKey,
        publicSignals,
        proof,
    );
...
function getPlonkTests() {
    return `
import assert from "assert";
import fs from "fs";
import { getInput } from "./input.js";
import {createRequire} from "module";
import plonkit from "../lib/plonkit/plonkit"

it("Test plonk prove and verify", async function(){
    const meta = JSON.parse(fs.readFileSync("circuits/compiled/meta.json","utf-8"))
    const input = await getInput();
    const require_ = createRequire(import.meta.url);
    const wcBuilder = require_(meta.witnessCalculatorPath);
    const witnessWasmBuff = fs.readFileSync(meta.witnessWasmPath);
    const wc = await wcBuilder(witnessWasmBuff);
    const wtns_buff = await wc.calculateWTNSBin(input,0);
    const setupfile_buff = fs.readFileSync(meta.setupFilePath);
    const r1cs_buff = fs.readFileSync(meta.r1csPath);
    const vkey_buff = fs.readFileSync(meta.vkeyPath);
    const proof = await plonkit.prove(setupfile_buff, r1cs_buff,wtns_buff,
        JSON.stringify(input, (_, value) => typeof value === 'bigint' ? value.toString() : value));
    const valid = await plonkit.verify(vkey_buff,proof);
...
function getJsLib(scheme, extraPublicInputs, hashfunc, merkletree, isPlonk) {
    if (scheme === "crs" || scheme === "crs-merkle") {
        const imports = getImports(hashfunc, scheme, isPlonk);
        const random = getRandom();
        const hashers = getHashers(hashfunc, merkletree, true);
        const generateProof = getGenerateProof(extraPublicInputs, hashfunc, merkletree, isPlonk);
        const verifyProof = getVerifyProof(isPlonk);
        ...
        const generateProof = eddsa_generateProof(extraPublicInputs, hashfunc, merkletree, scheme, isPlonk);
        const verifyProof = getVerifyProof(isPlonk);
...
module.exports = { getJsLib, getTests, getPlonkTests, getTestsHashersWithK, getTestsKDeclaration, addKToTestsCompute }