...
function eddsa_generateProof(publicInputsArr, hashfunc, merkletree, scheme,isPlonk) {
    let inputDocs = [];
    for (let i = 0; i < publicInputsArr.length; i++) {
...
` : ""
    ...
    let sparseTreeParamsDocs = "";
    let sparseTreeRootDocs = "";
    let sparseElementArgs = "";
    let sparseElementsFunctionArgs = ""
    if (scheme === "smt-eddsa-inc/exc") {
        sparseTreeParamsDocs = ` * @param {bigint} options.siblings - The merkle tree siblings
 * @param {bigint} options.oldValue - The old value
 * @param {bigint} options.oldKey - The old key
 * @param {bigint} options.isOld0 - The isOld parameter`
    }

    sparseTreeRootDocs = ` * @param {bigint } options.publicInputs.value - The value is the message signed and the value in the merkle tree
 * @param {bigint} options.publicInputs.root - The root of the merkle tree
 * @param {bigint} options.publicInputs.fnc - The fnc specifies the function if it's exclusion or inclusion proof`

    sparseElementArgs = `\n        siblings,\n        oldKey,\n        oldValue,\n        isOld0,\n`

    sparseElementsFunctionArgs = " siblings, oldValue, oldKey, isOld0,"
 ...
 * @param {bigint} options.isOld0 - The isOld parameter`
        ...
        sparseTreeRootDocs = `* @param {bigint } options.publicInputs.value - The value is the message signed and the value in the merkle tree
 * @param {bigint} options.publicInputs.root - The root of the merkle tree
 * @param {bigint} options.publicInputs.fnc - The fnc array specifies if it's insert/update/delete
 `
...
${merkletree === "sparse" ? sparseTreeParamsDocs : fixedTreeParamsDocs}
...
${merkletree === "sparse" ? sparseTreeRootDocs : fixedTreeRootDocs}
 ...
 */
export async function computeProof({ A, S, R8,${addTestsK(hashfunc, "justk")}${merkletree === "sparse" ? sparseElementsFunctionArgs : fixedPathElementsFunctionArgs}publicInputs, snarkArtifacts }) {
    const input = {
        //Private inputs
        A,
        S,
        R8,${merkletree === "sparse" ? sparseElementArgs : fixedPathElementsArgs}
    ...
    const { proof, publicSignals } = await ${isPlonk ? "plonk" :"groth16"}.fullProve(
        input,
 ...
 * @param {string} options.snarkArtifacts.zkeyFilePath - Path to the generated zKey file
 */
export async function computeProof({ Ax, Ay, S, R8x, R8y,${addTestsK(hashfunc, "justk")}${merkletree === "sparse" ? sparseElementsFunctionArgs : fixedPathElementsFunctionArgs}publicInputs, snarkArtifacts }) {
    const input = {
        //Private inputs
        Ax,
        Ay,
        S,
        R8x,
        R8y,${merkletree === "sparse" ? sparseElementArgs : fixedPathElementsArgs}
        ${addTestsK(hashfunc, "justk")} 
        //Public inputs
        ...publicInputs
    }
