...
function eddsa_generateProof(hashfunc, publicInputsArr) {
    if (hashfunc === "pedersen") {
 ...
 * @param {bigint | string} options.k - The key parameter for the MiMC7 hash
 * @param {Array<bigint> | Array<string>} options.pathElements - PathElements contains the merkle proof
 ...
 */
export async function computeProof({ Ax, Ay, S, R8x, R8y,${addTestsK(hashfunc, "justk")} pathElements, pathIndices, publicInputs, snarkArtifacts }) {
    const input = {
        //Private inputs
        Ax,
        Ay,
