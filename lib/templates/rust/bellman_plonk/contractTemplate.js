...
pub fn query(_deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    ...
    match msg {
        ...
        VerifyProof { proof_str } =>
            to_json_binary(
                &(VerifyProofResp {
                    verified: verify_proof(proof_str),
                })
            ),
    }
}
...
pub fn get_encoded_vkey() -> Vec<u8>{
    serde_json::from_str::<Vec<u8>>(r#"${enc_vkey}"#).expect("Invalid array string")
}
...
fn deserialize_proof_buff(proof_str : String) -> Vec<u8>{
    serde_json::from_str::<Vec<u8>>(proof_str.as_str()).expect("Invalid array string")
}
...
pub fn verify_proof(proof_str: String) -> bool {
    let vkey = load_verification_key::<Bn256>();
    let proof_bytes =  deserialize_proof_buff(proof_str);
...
mod tests{
    ...
    pub fn get_proof_str() -> &'static str {
       r#"${enc_proof}"#
    }

#[test]
fn test_verify_proof(){
    let proof_str = get_proof_str();
    
    let result = verify_proof(proof_str.to_string());
    assert!(result);
