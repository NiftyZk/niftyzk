const readmefile = {
    name: "readme.md", content: (isPlonk) => {
        return `# NiftyZK CLI
...
${isPlonk ? "\`niftyzk plonkfiles\` - Display information and download Trusted Setup files. These files were created for the Aztec Network ceremony as a phase 1 setup for plonk. Plonk requires no phase-2 ceremony unlike groth-16" :
...
${isPlonk ? `niftyzk gencontract --folder[string --circuit]  - Generate cosmwasm smart contract for verifying the zkp` : `\`niftyzk gencontract --bellman --ark --overwrite --folder[string] --circuit \` - Generate cosmwasm smart contract for verifying the zkp
