...
const { PTAUFILESDIR, UNIVERSALSETUPFILESDIR } = require('../paths');
...
const plonkSetupFiles = JSON.parse(`{
    "ListBucketResult": {
        ...
        "Contents": [
            {
                "Key": "setup_2^20.key",
                "LastModified": "2020-09-03T08:06:01.291Z",
                "ETag": "7cdda5f4c78c1baae21b6f1eb983d196",
                "Size": 67109136,
            ...
            {
                "Key": "setup_2^20_lagrange.key",
                "LastModified": "2020-09-03T08:05:59.991Z",
                "ETag": "c0d06547ce973910189f7c0281dd8558",
                "Size": 67109136,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^21.key",
                "LastModified": "2020-09-03T08:06:03.200Z",
                "ETag": "0f865b424635e453f592e47b8b6b5820-13",
                "Size": 134218000,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^21_lagrange.key",
                "LastModified": "2020-09-03T08:06:02.580Z",
                "ETag": "503b50efd9dc8c52d10ef0f423127e09-13",
                "Size": 134218000,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^22.key",
                "LastModified": "2020-09-03T08:06:05.069Z",
                "ETag": "4a7ae32479963059c6152b1fc5b1dbe7-26",
                "Size": 268435728,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^22_lagrange.key",
                "LastModified": "2020-09-03T08:06:03.907Z",
                "ETag": "ec4e71c16343700487fb0ea103c4a302-26",
                "Size": 268435728,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^23.key",
                "LastModified": "2020-09-03T08:06:07.219Z",
                "ETag": "66227427ec27dc7604e72139f18a82e0-52",
                "Size": 536871184,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^23_lagrange.key",
                "LastModified": "2020-09-03T08:06:06.188Z",
                "ETag": "31eea604168dc4dc5f3030c74af38281-52",
                "Size": 536871184,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^24.key",
                "LastModified": "2020-09-03T08:06:09.570Z",
                "ETag": "2d9c12d75bf48df7995d3b0b0ceee4a8-103",
                "Size": 1073742096,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^24_lagrange.key",
                "LastModified": "2020-09-03T08:06:08.480Z",
                "ETag": "7388345d99c3ec5d3501d4df44cb3d64-103",
                "Size": 1073742096,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^25.key",
                "LastModified": "2020-09-03T08:06:11.713Z",
                "ETag": "849cafdaef02f65bedf06dd36812ae77-205",
                "Size": 2147483920,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^25_lagrange.key",
                "LastModified": "2020-09-03T08:06:10.657Z",
                "ETag": "bff3bd8ecf083d5cd8fefde874dd5367-205",
                "Size": 2147483920,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^26.key",
                "LastModified": "2020-09-03T08:06:14.000Z",
                "ETag": "945a53e51cb0b80c0341967121dd0d5b-410",
                "Size": 4294967568,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
            ...
            {
                "Key": "setup_2^26_lagrange.key",
                "LastModified": "2020-09-03T08:06:12.850Z",
                "ETag": "f80858177f408f7f07bcb0c42d4f3573-410",
                "Size": 4294967568,
                "StorageClass": "STANDARD",
                "Owner": {
                    "ID": "5048583",
...
function isValidPtauFilename(selectedName) {
    for (let i = 0; i < ptauFiles.length; i++) {
        if (selectedName === ptauFiles[i].name) {
            return true;
        }
    }
    return false;
...
function isValidPlonkSetupFileName(selectedName) {
    for (let i = 0; i < plonkSetupFiles.ListBucketResult.Contents.length; i++) {
        if (selectedName === plonkSetupFiles.ListBucketResult.Contents[i].Key) {
            return true
        }
    }
    return false
...
function getDownloadLink(name) {
    return `https://storage.googleapis.com/zkevm/ptau/${name}`
}

function getDownloadLinkForPlonkitFiles(name) {
    return `https://universal-setup.ams3.digitaloceanspaces.com/${name}`
}
...
function explainPtauFiles() {
    ...
    for (let i = 0; i < ptauFiles.length; i++) {
        ...
        console.log(chalk.cyan(ptauFiles[i].name))
        console.log(`blake2b hash: ${ptauFiles[i].hash}`)
        console.log(`Power: ${ptauFiles[i].power}, Max Constraints: ${ptauFiles[i].maxConstraints}`)
        console.log(chalk.yellow(`Size: ${formatBytes(ptauFiles[i].size)}`))
        console.log(`Supports built in ceremony server: ${i < 8 ? chalk.green("YES") : chalk.yellow("File too large. Requires manual ceremony.")}`)
        console.log("\n")
    }
...
function explainPlonkitFiles() {
    console.log("Available setup files for PLONK\n")
    for (let i = 0; i < plonkSetupFiles.ListBucketResult.Contents.length; i++) {
        console.log(chalk.cyan(plonkSetupFiles.ListBucketResult.Contents[i].Key))
        console.log(chalk.yellow(`Size: ${formatBytes(plonkSetupFiles.ListBucketResult.Contents[i].Size)}`))
        console.log("\n")

    }
}

async function downloadPtauFile(ptauFileName) {
    if (!Fs.existsSync(PTAUFILESDIR)) {
        Fs.mkdirSync(PTAUFILESDIR)
    }
    const url = getDownloadLink(ptauFileName)
    console.log("Connecting to download " + ptauFileName);
    ...
    const fileSize = ptauFiles.find(file => file.name === ptauFileName).size

    console.log("Starting to download " + formatBytes(fileSize))
    ...
    const writer = Fs.createWriteStream(
        Path.resolve(process.cwd(), "ptau", ptauFileName)
    )
    data.on("data", (chunk) => {
        progressBar.tick(chunk.length)
    })
    // data.on("end", function () { console.log("\n") })
    data.pipe(writer)
}

async function downloadUniversalSetupFiles(filename) {
    if (!Fs.existsSync(UNIVERSALSETUPFILESDIR)) {
        Fs.mkdirSync(UNIVERSALSETUPFILESDIR)
    }
    const url = getDownloadLinkForPlonkitFiles(filename)
    console.log("Connecting to download " + filename);
    const { data, headers } = await Axios({
        url,
        method: "GET",
        responseType: "stream"
    ...
    const totalLength = headers['content-length']
    const fileSize = plonkSetupFiles.ListBucketResult.Contents.find(file => file.Key === filename).Size
    console.log("Starting to download " + formatBytes(fileSize))
    const progressBar = new ProgressBar('downloading [:bar] :percent :etas', {
        width: 20,
        complete: '=',
    ...
    const writer = Fs.createWriteStream(
        Path.resolve(process.cwd(), "setupfiles", filename)
    )
    data.on("data", (chunk) => {
        progressBar.tick(chunk.length)
    })
...
async function selectPtauFileToDownload() {
    inquirer.prompt([
        {
            type: "list",
            name: "ptaufile",
            message: "Select a ptau file to download",
            choices: ["cancel"].concat(ptauFiles.map((file) => file.name))
        }
    ]).then(async (answers) => {
        ...
        await downloadPtauFile(filename)
    }).catch((err) => {
...
async function selectPlonkSetupileToDownload() {
    inquirer.prompt([
        {
            type: "list",
            name: "universalsetup",
            message: "Select a universal setup file to download",
            choices: ["cancel"].concat(plonkSetupFiles.ListBucketResult.Contents.map((file) => file.Key))
        }
    ]).then(async (answers) => {
        const filename = answers.universalsetup;
        if (filename === "cancel") {
            return
        }
        await downloadUniversalSetupFiles(filename)
    }).catch((err) => {
        console.log(err)
        console.log("Download aborted")
    })
...
module.exports = { explainPlonkitFiles, downloadUniversalSetupFiles, selectPlonkSetupileToDownload, isValidPlonkSetupFileName, ptauFiles, getDownloadLink, formatBytes, explainPtauFiles, downloadPtauFile, selectPtauFileToDownload, isValidPtauFilename }